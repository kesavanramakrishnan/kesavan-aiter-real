...
configs.append(
    triton.testing.Benchmark(
        ...
        x_vals=[
            ...
            # (True, 2, 32, 8, 8192, [8192, 8192], 128, 608, torch.float16, 128, 64, 2, 4),
            # (True, 2, 64, 8, 8192, [8192, 8192], 128, 608, torch.float16, 128, 64, 2, 4),
            # (True, 2, 128, 8, 8192, [8192, 8192], 128, 608, torch.float16, 128, 64, 2, 4),
            # (True, 2, 32, 8, 32768, [32768, 32768], 128, 608, torch.float16, 128, 64, 2, 4),
            # (True, 2, 64, 8, 32768, [32768, 32768], 128, 608, torch.float16, 128, 64, 2, 4),
            # (True, 2, 128, 8, 32768, [32768, 32768], 128, 608, torch.float16, 128, 64, 2, 4),
            # Experiment 1: MHA (non-causal), batch in {1,2,4,8}, N_CTX in {8K,32K,128K}, H_Q in {8,16,32,64,128}
            *[
                (
                    True,  # causal
                    b,      # batch
                    h,      # hq
                    h,      # hk (MHA: hk == hq)
                    n_ctx,     # n_ctx_q (keep small to bound buffer sizes)
                    [n_ctx] * b,  # n_ctx list per request
                    128,    # d
                    608,    # total_programs
                    torch.float16,  # init_dtype
                    128,     # BLOCK_M
                    64,    # BLOCK_N
                    2,      # waves_per_eu
                    4,      # num_warps
                ...
                for b in [1, 2, 4, 8]
                for n_ctx in [8192, 32768, 131072]
