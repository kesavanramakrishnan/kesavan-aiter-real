...
from aiter.ops.triton.lean_atten_bwd_clean import la_backward_persistent as la_backward_persistent
...
def run_test_and_benchmark(config: dict):
    ...
    print("  Warming up and benchmarking Lean (naive) bwd kernel...")
    # Convert BHSD -> BSHD for the naive kernel
    ...
    dq_la_bshd = torch.empty_like(q_bshd)
    dk_la_bshd = torch.empty_like(k_bshd)
    dv_la_bshd = torch.empty_like(v_bshd)
    ...
    atol, rtol = 1e-2, 1e-2
    # Convert naive outputs from BSHD -> BHSD for comparison
    dq_la, dk_la, dv_la = (
        dq_la_bshd.transpose(1, 2),
        dk_la_bshd.transpose(1, 2),
        dv_la_bshd.transpose(1, 2),
    )
    la_passed = all([
