...
from .utils.index_max_tiles import precompute_lean_attention_indices
...
def persistent_lean_attention(
    ...
    max_output_tile_cnt, tile_indices = precompute_lean_attention_indices(
        causal=causal,
        batch_size=batch_size,
        max_seqlen_q=Op.shape[1],
    ...
    if tile_indices is None:
        # Create a dummy tensor to pass to the kernel. Its contents don't matter
        # for non-causal attention, but it must be a valid tensor to avoid errors.
        tile_indices = torch.empty((1, 1), dtype=torch.int32, device=q.device)
    ...
    la_kernel = la_persistent[grid](
        ...
        locks,
        tile_indices,
        tile_indices.stride(0),
        tile_indices.stride(1),
        q.stride(0),  # N_CTX_Q
        q.stride(1),  # H
        q.stride(2),  # Head_Dim
        k.stride(0),
...
def la_persistent(
    ...
    locks,
    Tile_Indices,
    stride_ti_s, # Stride for the first dimension
    stride_ti_d, # Stride for the second dimension
    stride_qm,  # n_ctx_q
    stride_qh,  # Head
    stride_qk,  # head_dim
    stride_kn,
...
def la_persistent_inner(
    ...
    if causal:
        ...
        # 2. Calculate the specific index for the lookup tensor. This is the tile's
        #    position within a single head and single batch item's workload.
        lookup_idx = iter - (tile_head_idx * tiles_per_head) - (tile_batch_idx * (tiles_per_head // batch_size))
        
        # 3. Calculate the base pointer to the correct row in the lookup tensor.
        indices_ptr = Tile_Indices + lookup_idx * stride_ti_s
        
        # 4. Load the three pre-computed values directly from global memory.
        #    This is much faster than the iterative find_group function.
        per_head_tile_idx = tl.load(indices_ptr + 0 * stride_ti_d)  # Column 0: q_block_idx
        per_head_tile_size = tl.load(indices_ptr + 1 * stride_ti_d) # Column 1: task_size
        total_blocks = tl.load(indices_ptr + 2 * stride_ti_d)      # Column 2: start_offset
        tile_iter = (
